# NCZ 26.5 r77 "Reinhardt-Magnetar" — Unified Desktop/Server ISO

**Release date:** 2026-05-07 (planned)
**Codename:** Reinhardt (Desktop) + Magnetar (Server) on a single bootable ISO
**Kernel:** linux-cix-sky1 6.18.26 LTS (default) + 7.0.3-cix-sky1-next (boot-counting auto-rollback)
**Userspace:** Ubuntu 25.10 questing arm64
**Hardware:** Cix Sky1 / CP8180 (Minisforum MS-R1, Radxa Orion O6, Framework 13 ARM mainboard)
...
- **Magnetar from r75 -> r77** — direct upgrade path: re-flash the new ISO, reinstall. No in-place upgrade today (the rootfs path doesn't support it).
- **Reinhardt from r74 -> r77** — same.
...
*This release is the union of the r75 Magnetar work + the r76 netinstall design + the r77 unified-chooser implementation. Netinstall mode (sub-500 MB ISO via ports.ubuntu.com debootstrap) remains queued for r78 — see `docs/R76-NETINSTALL-DESIGN.md`.*
