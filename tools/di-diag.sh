...
if [ "$CMD" = "diag" ]; then
    CMD='echo DI_PHASE_UNAME; uname -a; \
echo DI_PHASE_CHROOT_ERRORS; grep -iE "fatal|fail|chroot|target|cdebootstrap" /var/log/syslog 2>/dev/null | tail -50; \
echo DI_PHASE_TARGET_TREE; ls -la /target 2>&1 | head -25; ls /target/bin /target/usr/bin 2>&1 | head -10; \
echo DI_PHASE_MOUNTS; mount | head -30; \
echo DI_PHASE_DF; df -h | head -15; \
echo DI_PHASE_EARLY_SENTINEL; cat /etc/early_command_ran.txt 2>&1; \
echo DI_PHASE_EARLY_LOG; tail -40 /var/log/early_command.log 2>&1; \
echo DI_PHASE_INSTALLER_LOG; tail -100 /var/log/installer/syslog 2>/dev/null; ls /var/log/installer/ 2>&1; \
echo DI_PHASE_SYSLOG; tail -80 /var/log/syslog; \
echo DI_PHASE_END'
fi
