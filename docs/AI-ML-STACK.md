<p align="center">
  <img src="../assets/branding/wallpaper/ncz-wallpaper-05-magnetar-jets-2k.jpg" alt="" width="880">
</p>

# NCZ Agentic Linux — AI/ML Stack Reference (Cix Sky1)

> **Scope note (26.7).** NCZ-OS ships one kernel, `7.2.0-sky1-ncz`, and
> **both GPU drivers**:
>
> | Driver | Package | Role | Compute API |
> |---|---|---|---|
> | `mali_kbase` | `cix-gpu-kmd` | **Default** | OpenCL via CIX `libmali` |
> | `panthor` | `panthor-cix` | Selectable | Vulkan/OpenCL via Mesa PanVK/rusticl |
>
> Both are installed and both are blacklisted in
> `/etc/modprobe.d/ncz-gpu-drivers.conf`, so neither binds by accident; one is
> selected explicitly and `ncz-gpu-select` switches between them.
>
> **The GPU numbers in §3 and the GPU rows of §0/§6 were measured on the
> panthor + Mesa rusticl path and have not been re-measured on
> `mali_kbase`.** Read them as panthor-path figures. The CPU, NPU and VPU
> material is driver-independent and current.
>
...
> MC10** GPU (`mali_kbase` by default, `panthor` selectable), **ArmChina
> Zhouyi V3** NPU, **Linlon/amvx** VPU.
...
| `cix-gpu-umd` → `libmali.so`, `libOpenCL.so` | `/opt/cixgpu-pro/` | ✅ **CIX vendor Mali driver** — active by default, providing GLES 3.2 and OpenCL. Mesa rusticl/panvk is the alternative, used when `panthor` is selected. See §3. |
...
for the ctypes pattern — there is no `vision_demo.py` in this repo; use the
`libnoe` pattern against the shipped ResNet50 test graphs in
`/usr/share/cix/testdata/npu/` directly).

### 5.3 Pulling new models from the repo
Two distinct repos, don't confuse them (see
[docs/MODELSCOPE-MODELS.md](MODELSCOPE-MODELS.md) for the full breakdown):
`cixtech/ai_model_hub` on GitHub is **recipes only** (no `.cix` blobs); the
actual prebuilt `.cix` binaries live at `cix/ai_model_hub` on modelscope.cn,
versioned by quarter (current: **`26_Q1`**).

```bash
# (a) NCZ CLI helper — NOTE: `ncz models pull` is a stub as of this writing
# (post-install/46-ncz-cli.sh prints "STUB (r75)" and exits 2). Use (b) below
# until it's implemented.
ncz models list             # show installed .cix files -- this one works
...
All numbers measured on the `.66` board on the kernel line that preceded 7.2
(`7.0.12-cix-sky1-next`), which has since been removed from the image. They
have not been re-measured on `7.2.0-sky1-ncz`;
Mesa 26.1.3.
...
# hard-pin instead (appliance use, deterministic):
