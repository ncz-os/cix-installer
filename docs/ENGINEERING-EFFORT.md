# Engineering effort log — CIX Sky1 bring-up (26.6 era)

> **This is a historical document.** It records the bring-up effort for
> **NCZ-OS 26.6 "Reinhardt"** and describes that release's world: a
> multi-channel kernel programme (7.2 / 7.0.12 / 6.18), separate Reinhardt
> desktop and Magnetar server variants, and an Ubuntu base.
>
> **None of that describes 26.7.** The current release ships one kernel
> (`7.2.0-sky1-ncz`, with 7.0.12 removed from the image), one build, and a
> Debian Forky base. For the current position see
> [HOW-DID-WE-GET-HERE.md](HOW-DID-WE-GET-HERE.md) and
> [DESIGN-RATIONALE.md](DESIGN-RATIONALE.md).
>
> It is kept because the measurements and the failure analysis in it are real
> and still instructive — particularly the patch-integration arithmetic and the
> A/B methodology against a known-good baseline.

> **This is a historical log, frozen at the 26.6 (r112-era) bring-up.** It
> predates the Singularity Desktop migration, the `mali_kbase` GPU driver
> port, and the three-tier (7.2/7.0.12/6.18) kernel-channel model that ship
> in 26.7 "Maximilian" — e.g. §6's "XFCE on LightDM" and Plymouth theme, and
> the two-kernel A/B framing throughout, describe what was true at the time
> and are **not** current. For current state, see the top-level `README.md`
> and [`docs/HOW-DID-WE-GET-HERE.md`](HOW-DID-WE-GET-HERE.md).
>
...
- **2 kernels maintained side‑by‑side**: `6.18.26-cix-sky1-lts` (production
  default) and `7.0.12-cix-sky1-next` (beta/edge), plus a rescue target —
  all selectable from the rEFInd menu. *(Written when the bootloader was
  still systemd-boot with 3-try automatic rollback; switched to rEFInd at
  r118, which has no boot-counting -- rollback is manual now, via the
  rescue partition.)*
- **~195 kernel patches** curated/hand‑merged across the two trees (140 for
  LTS, 55 for NEXT, as of the 2026-07-05 meta-cix consolidation — Sky1‑Linux
...
- `module_blacklist=typec_rts5453,rts5453` — **inert, and retained only as
  history.** It dates from an earlier kernel where the Type-C PD driver
  reportedly wedged boot on an IRQ-151 conflict. That does not reproduce on
  7.2, where the driver is built in (`CONFIG_TYPEC_RTS5453=y`) and
  `module_blacklist` therefore has no effect on it. PD works: see the driver
...
The official kernel is built as a **new sibling** so the proven edge stays
bootable as fallback while we validate. A forward **7.1 track** is scaffolded
(patches need porting from v7.0 → v7.1; first conflict at
`drivers/clk/Kconfig`).

*(Update: the recipe this became is `linux-cix-sky1-official_7.1.1.bb` --
based on v7.1.1, not 7.0.12, carrying 74 of CIX's 76 patches (2 dropped:
mailbox IRQF_NO_SUSPEND, mainlined in 7.1; the pl011 BSP fork, superseded by
mainline 7.1 pl011) plus 6 NCZ forward-port patches, 80 total. The 7.1 track
itself did not pan out as a shipping kernel -- see README.md → *Current ISO*
for why; a 7.2 migration is now the active target.)*
