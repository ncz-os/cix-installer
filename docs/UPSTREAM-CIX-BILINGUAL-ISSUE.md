...
**核心结论:** Cix Sky1 NPU 在硅片层面与当前 Intel 集显 + OpenVINO 2026.1.0 在此工作负载上**统计学上持平**。是工具链阻碍它成为这个硅片级别的首选。我们识别了五个具体缺口,希望团队反馈正确的解决路径。
...
冷启动时 Cix NPU 比 Intel 集显慢 7% — 这是软件调优差距,不是硅片问题。在真实的 50% 重复混合工作负载下,两者统计学上持平。
