...
#   in run-all.sh's EXIT trap, and WIPES /boot/efi/loader/entries/*.conf +
#   vmlinuz-* every run. So the ESP staging + the cixmini-rescuepart.conf
#   loader entry are written by 70-bootloader.sh (the sole ESP owner), gated
#   on the RESCUE_READY marker this hook leaves behind. Here we only touch the
#   rescue PARTITION (never the ESP), so nothing we do gets wiped.
#
