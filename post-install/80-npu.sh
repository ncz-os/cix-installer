...
# The KVER_NEXT sidecar is authoritative and, on a normal install, carries
# the edge kernel (assets/kernel/edge -> KVER_NEXT), so 7.2 gets the SSDT
# overlay through the normal edge path below. This fallback only covers the
# case where a 7.2-class NCZ kernel is present on disk but NOT represented by
# the sidecar (e.g. a hand-deployed 7.2 .deb, or sidecar drift): without it
# that kernel's initrd would miss the CIXH4010 _HID overlay and the NPU cores
# would hard-hang on first register read. De-duped against the sidecar (adds
# nothing on a normal install where 7.2==KVER_NEXT) and idempotent downstream
# (the SSDT prepend checks newc-magic first).
for _mk in /lib/modules/*7.2*-ncz /usr/lib/modules/*7.2*-ncz; do
    ...
    [ "$_known" = "1" ] && continue
    echo "[80] fallback: NCZ edge kernel $_kv present but not in KVER_NEXT sidecar — folding in for NPU SSDT"
    KERNEL_LABELS+=("edge")
    KERNEL_VERS+=("$_kv")
done

if [ "${#KERNEL_VERS[@]}" -eq 0 ]; then
    echo "[80] WARN: no KVER_NEXT sidecar found; skipping kernel-specific NPU setup"
fi
if [ ! -d "$ASSETS" ]; then
    # assets/npu/ is a gitignored blob dir carrying two OPTIONAL payloads: the
    # out-of-tree armchina_npu-<KVER>.ko (only needed when the kernel does not
    # build the driver in-tree) and the MS-R1 ACPI SSDT override. Both are
    # board/kernel-specific, so an absent directory is not automatically a
    # problem — say what is actually being skipped instead of a bare WARN.
    echo "[80] NPU asset directory not staged at $ASSETS"
    echo "[80]   -> no out-of-tree armchina_npu-<KVER>.ko (fine if the kernel ships it in-tree)"
    echo "[80]   -> no MS-R1 ACPI SSDT override (fine on boards with a native CIXH4010 _HID)"
fi
...
# --- Step 1b: ensure the matched aipu DKMS module (cix-npu-driver-dkms, installed
# by 25-cix-proprietary.sh) is built for each TARGET kernel. dpkg's dkms postinst
# runs `dkms autoinstall` over /lib/modules/*/build dirs; be explicit per target
# KVER so an in-target install (chroot uname != target KVER) still yields
# /usr/lib/modules/$KVER/updates/dkms/aipu.ko. Idempotent; leaves the SSDT +
# power/control=auto lifecycle to the driver (never force power on). ---
if command -v dkms >/dev/null 2>&1 && dkms status 2>/dev/null | grep -qE '^aipu[,/]'; then
    AIPU_VER=$(dkms status 2>/dev/null | sed -n 's#^aipu[,/] *\([0-9][0-9.]*\).*#\1#p' | head -1)
    for KVER in "${KERNEL_VERS[@]}"; do
        [ -d "/usr/lib/modules/$KVER" ] || continue
        if ls /usr/lib/modules/$KVER/updates/dkms/aipu.ko* >/dev/null 2>&1; then
            echo "[80] aipu dkms already built for $KVER"
        elif [ -d "/usr/lib/modules/$KVER/build" ]; then
            echo "[80] building aipu dkms ${AIPU_VER:-6.2.0} for $KVER..."
            dkms install -m aipu -v "${AIPU_VER:-6.2.0}" -k "$KVER" --force 2>&1 | tail -4 || echo "[80] WARN: aipu dkms build failed for $KVER"
            depmod -a "$KVER" 2>/dev/null
        else
            echo "[80] WARN: no headers at /usr/lib/modules/$KVER/build - cannot build aipu dkms for $KVER"
        fi
    done
fi
...
if [ -f "$SSDT_CPIO" ] && should_apply_npu_ssdt; then
    ...
    # 2026-08-22: update-initramfs REGENERATES /boot/initrd.img-$KVER from
    # scratch, silently DROPPING the prepended SSDT early-CPIO. That is exactly
    # how the MS-R1 lost its NPU _HID override on the first on-device kernel/
    # DKMS update: the regenerated initrd booted without the SSDT, the CRE
    # cores had no platform devices, and (pre-+ncz3) sky1_npu_probe panicked
    # in pm_runtime_enable(NULL). Install an initramfs post-update hook so
    # every future regeneration re-prepends the override from the copy
    # persisted at /boot/npu-acpi-override.cpio above.
    mkdir -p /etc/initramfs/post-update.d
    cat > /etc/initramfs/post-update.d/zz-npu-ssdt << 'NPUHOOK'
#!/bin/sh
# Re-prepend the NCZ NPU ACPI SSDT override after update-initramfs.
# Args: $1 = kernel version, $2 = initramfs path.
# Installed by cix-installer post-install/80-npu.sh (MS-R1-class boards only).
CPIO=/boot/npu-acpi-override.cpio
[ -f "$CPIO" ] || exit 0
INITRD="$2"
[ -n "$INITRD" ] && [ -f "$INITRD" ] || exit 0
# Already prepended? (early-CPIO = uncompressed newc archive, magic 070701)
head -c 6 "$INITRD" 2>/dev/null | grep -q '070701' && exit 0
TMP="${INITRD}.npu-ssdt.$$"
if cat "$CPIO" "$INITRD" > "$TMP" && mv "$TMP" "$INITRD"; then
    echo "npu-ssdt: re-prepended ACPI override to $INITRD" >&2
else
    rm -f "$TMP"
    echo "npu-ssdt: WARNING: failed to prepend override to $INITRD" >&2
fi
exit 0
NPUHOOK
    chmod 0755 /etc/initramfs/post-update.d/zz-npu-ssdt
    echo "[80] installed /etc/initramfs/post-update.d/zz-npu-ssdt (SSDT survives initrd regeneration)"
elif [ ! -f "$SSDT_CPIO" ]; then
    # 2026-08-02: this used to warn unconditionally, which produced a scary
    # "NPU cores may not enumerate" line on every O6/O6N install even though
    # should_apply_npu_ssdt() would have SKIPPED the override on those boards
    # anyway (their firmware exposes the CIXH4010 _HID natively — the SSDT is a
    # workaround for the Minisforum MS-R1 BIOS gap only). Ask the detector
    # first, so the warning only fires on a board that actually needed the
    # asset, and is silent-but-explicit everywhere else.
    if should_apply_npu_ssdt; then
        echo "[80] WARN: SSDT CPIO missing at $SSDT_CPIO — this board NEEDS the ACPI _HID"
        echo "[80]       override, so the NPU cores will NOT enumerate. Stage the override at"
        echo "[80]       assets/npu/npu-acpi-override.cpio and rebuild the ISO."
    else
        echo "[80] SSDT CPIO not staged at $SSDT_CPIO — not needed on this board (native _HID)"
    fi
else
...
#
# The module is aipu.ko, NOT armchina_npu. "armchina_npu" is the IN-TREE
# driver's name, and the defconfig deliberately leaves CONFIG_ARMCHINA_NPU
# unset so the in-tree copy cannot mask the vendor DKMS build. Naming it here
# produced "systemd-modules-load: Failed to find module 'armchina_npu'" on
# every boot (measured on cixmini 2026-08-16) while doing nothing useful.
aipu
EOF
...
# NCZ 26.7 "Maximilian" — Cix Sky1 / Zhouyi v3 NPU

## What works

- **Kernel module** `armchina_npu.ko` probes clean on MS-R1 for the
  shipped NCZ kernel:
  - 7.2.0-sky1-ncz (edge; in-tree CONFIG_ARMCHINA_NPU=m)
