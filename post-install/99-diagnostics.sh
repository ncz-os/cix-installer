...

{
    ...
    echo ""
    for f in RELEASE BUILD_VERSION BUILD_DATE BUILD_HOST KVER_NEXT; do
...
| 60-boot-splash.log | Native KMS boot splash |
...
| /etc/cix-installer/KVER_NEXT | edge kernel uname -r (the only shipping kernel) |
