...
set +e
LOG=/var/log/early_command.log
# Follow the release profile for the on-media suite; the ISO ships
# dists/<NCZ_BASE_CODENAME>, so a hard-coded suite makes every apt-get update
# against file:///cdrom fail with "File not found - /cdrom/dists/<suite>/...".
NCZ_BASE_CODENAME=""
for _rel in /cdrom/cixmini/RELEASE /hd-media/cixmini/RELEASE \
            /media/cdrom/cixmini/RELEASE /run/live/medium/cixmini/RELEASE; do
    [ -r "$_rel" ] && . "$_rel" && break
done
INDEX=/cdrom/dists/${NCZ_BASE_CODENAME:-resolute}/main/binary-arm64/Packages
...
# Debian pkgsel ships /usr/lib/pre-pkgsel.d/90popcon, which unconditionally
# runs `apt-install --no-recommends popularity-contest` before pkgsel/include is
# considered. In the baked-rootfs flow pkgsel is supposed to be a no-op, and on
# forky DIAG runs popcon's postinst fails under d-i with fd 3 closed. Rewrite
# that hook immediately before pkgsel reaches it.
install_ncz_disable_popcon_hook() {
    hook_dir=/usr/lib/pre-pkgsel.d
    hook_tmp="$hook_dir/.00ncz-disable-popcon.$$"
    hook_final="$hook_dir/00ncz-disable-popcon"

    mkdir -p "$hook_dir"
    rm -f "$hook_tmp"
    cat > "$hook_tmp" <<'NCZNOPOPCON'
#!/bin/sh
set +e
LOG=/var/log/early_command.log
cat > /usr/lib/pre-pkgsel.d/90popcon <<'EOF'
#!/bin/sh
exit 0
EOF
chmod 0755 /usr/lib/pre-pkgsel.d/90popcon 2>/dev/null || true
echo "[ncz-disable-popcon] disabled pkgsel 90popcon hook" >> "$LOG"
exit 0
NCZNOPOPCON

    if grep -q '^exit 0$' "$hook_tmp" && chmod 0755 "$hook_tmp" && mv -f "$hook_tmp" "$hook_final"; then
        echo "[watcher] installed ncz disable-popcon hook for pre-pkgsel.d"
    else
        rm -f "$hook_tmp"
        echo "[watcher] failed to publish $hook_final"
    fi
...
while :; do
    ...
    if [ "$SSHD_PATCHED" -eq 0 ] && [ -f /etc/ssh/sshd_config ] && pidof sshd >/dev/null 2>&1; then
        ...
        if grep -q '^PermitRootLogin yes' /etc/ssh/sshd_config \
           && grep -q '^PubkeyAuthentication yes' /etc/ssh/sshd_config \
           && grep -q '^PasswordAuthentication yes' /etc/ssh/sshd_config \
           && pidof sshd >/dev/null 2>&1; then
            echo "[watcher] sshd patch VERIFIED $(date -u +%FT%TZ) — continuing /target resolv.conf sync"
            SSHD_PATCHED=1
        else
            echo "[watcher] ERROR: sshd patch/reload verification failed; will retry"
        fi
