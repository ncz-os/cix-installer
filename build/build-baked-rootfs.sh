...
BASE="${BASE:-$ROOT/assets/rootfs/rootfs-$NCZ_BASE_CODENAME-arm64.tar.zst}"
VARIANT="${VARIANT:-desktop}"
NCZ_TARGET_ARCH="${NCZ_TARGET_ARCH:-$(dpkg --print-architecture)}"
WORK="${WORK:-/tmp/ncz-bake}"
OUT="${OUT:-$ROOT/assets/rootfs/rootfs-$NCZ_BASE_CODENAME-arm64-baked.tar.zst}"
CHROOT="$WORK/chroot"
LOGDIR="$WORK/logs"

# Hooks to BAKE at build time (generic: drivers, desktop, branding,
# apt-config). Machine-specific hooks stay at install: 09-diag-account (user),
# 33-network, 34-fstab (disk UUIDs), 37-failsafe, 38-recovery, 48-server-variant,
...
20-desktop 22-display-fix 25-cix-proprietary 26-gpu-default-open 27-net-caps \
31-remote-access 32-quadlet-shim 35-fstrim-fix 35-ssh 36-telemetry \
39-usb-gpt-autofix 40-claude-code 45-wallpaper-rotator 46-ncz-cli 46-python311 \
47-embedkit 47-llm-stack 50-brand 56-icon-theme 60-boot-splash \
80-npu 92-buildkite-apt"
...
case "$NCZ_TARGET_ARCH" in
    arm64|amd64) ;;
    *) echo "FATAL: unsupported NCZ_TARGET_ARCH=$NCZ_TARGET_ARCH" >&2; exit 1 ;;
esac

log "FULL BAKE start — variant=$VARIANT target_arch=$NCZ_TARGET_ARCH base=$BASE"
...
for _d in cix-debs kernel rescue refind branding models python311 gpu mgmt diag npu wallpaper; do
...
[ -f "$ROOT/assets/ncz-cli.sh" ] && sudo cp -a "$ROOT/assets/ncz-cli.sh" "$CHROOT/usr/local/lib/cix-installer/assets/" 2>/dev/null || true
...
printf '%s' "$NCZ_TARGET_ARCH" | sudo tee "$CHROOT/usr/local/lib/cix-installer/BUILD_ARCH" >/dev/null
# stage kernel-version sidecar so 10-our-kernel finds it (build-iso-di.sh
# stages this at ISO time; the bake must too).
...
deb ${NCZ_BASE_MIRROR} ${NCZ_BASE_CODENAME} ${NCZ_BASE_COMPONENTS}
APT
...
for _g in cix-debs models python311 gpu mgmt npu wallpaper branding; do sudo rm -rf "$A/$_g" 2>/dev/null; done
