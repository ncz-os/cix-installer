...
Archive: stable
Origin: nclawzero
Label: nclawzero-cixmini-$NCZ_BASE_CODENAME
Version: 1.0
Acquire-By-Hash: yes
Component: $component
Architecture: $arch
...
PKGS=(
  ...
  ubuntu-server openssh-server openssh-client telnetd inetutils-telnetd openbsd-inetd
  ...
  # 26.7: Singularity Desktop (Wayland/GTK4) replaced XFCE. The desktop runtime
  # closure is built by build-desktop-mirror.sh (manifests/desktop.pkgs). Only
  # the greeter + Wayland session infra are seeded here.
  lightdm lightdm-gtk-greeter
  foot xsettingsd seatd swaybg xwayland

  # Login (NO plymouth — native KMS singularity-boot-splash replaces it)
  gdm3
  xrdp
  ...
  # Dev tooling
  build-essential git neovim tmux gdb jq bc fdisk parted parted e2fsprogs dosfstools
  ...
  # Bluetooth + audio
  bluez bluetooth bluez-tools pulseaudio-module-bluetooth

  # AI and Machine Learning
  python3-numpy python3-scipy python3-pil python3-requests python3-protobuf python3-tqdm python3-yaml
...
DEB_COUNT=$(find "$CHROOT/var/cache/apt/archives" -maxdepth 1 -type f -name '*.deb' | wc -l)
echo "[mirror] downloaded $DEB_COUNT .debs"
echo "[mirror] total size: $(du -sh --exclude=partial "$CHROOT/var/cache/apt/archives" | cut -f1)"
