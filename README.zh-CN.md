...
> **🤖 AI 智能体为可选启用 —— 本发行版是「具备智能体能力」，而非「默认开启智能体」。**
> 全新安装即是一套完整可用的 Linux 桌面（驱动、GPU、NPU、音频均可正常工作），**不会
> 安装或运行任何智能体**。运行时、quadlet 以及设备端 NPU 记忆栈都已**预置就绪**，但没有
> 任何智能体或记忆服务会在开机时自启动或自行联网拉取。你需要在终端中运行 **`ncz`** 来主动
> 启用：它才会安装 AI 智能体（`zeroclaw`，以及可选的 `openclaw`、`hermes`、`portainer`
> 和 NVIDIA NemoClaw）和 **MNEMOS** 记忆系统。在你运行它之前，系统的行为与任何普通 Linux
> 桌面无异。（早期 ISO 会默认激活 `zeroclaw`；当前 ISO 不再如此。）
...
| **Radxa Orion O6 / O6N** | Cix Sky1 | ❌ **未测试 —— 我们非常需要测试者。** 相同的 SoC，但*板卡不同*（设备树、PMIC、BIOS、外设都不同）。我们**没有**这台机器。*网络问题已修复：* O6 的 Realtek 网卡（RTL8125/8126）现已在安装器和已安装系统中同时附带 `rtl_nic` 固件，解决了此前无网络的回归问题 —— 但整块板卡仍未经验证。**如果你有 O6：请安装、测试并提交 issue。如果 Radxa 的同仁看到这里 —— 我们需要一台板子。** |
...
4. `30-agents.sh` —— 安装 podman + `ncz` CLI + 预置智能体 quadlet 模板与 OCI 镜像（不激活任何智能体；全部通过 `ncz` 可选启用）
...
**26.6（r126）** —— 当前发布。开源 Mesa 26.1.3 栈现已成为 **OpenGL/GLX、Vulkan
和 OpenCL** 的完整默认 GPU 提供方；CIX 专有栈仍保留在磁盘上（`.disabled`），
供将来的可选切换器使用。仅在 Minisforum MS-R1 上测试 —— 见上文**硬件支持与测试
状态**。**Reinhardt**（桌面）和 **Magnetar**（服务器）两个变体都从这棵代码树构建。
